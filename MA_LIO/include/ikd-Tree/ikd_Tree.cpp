    ...
    for (int i = 0; i < PointToAdd.size(); i++)
    {
        if (downsample_switch)
        {
            ...
            for (int index = 0; index < Downsample_Storage.size(); index++)
            {
                ...
                if(tmp_dist < downsample_size/8 && min_dist < downsample_size/8){
                    if(Downsample_Storage[index].normal_y < min_cov){
                        min_dist = tmp_dist;
                        min_cov = Downsample_Storage[index].normal_y;
